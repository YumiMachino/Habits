...
extension HomeCollectionViewController {
    ...
    func updateCollectionView() {
        ...
        .reduce(into: [ViewModel.Item]()) { partial, statistic in
            ...
            // Find the index of the current user's count, keeping in mind that it
            //  won't exist if the user hasn't logged that habit yet.
        ...
        for followedUser in model.followedUsers.sorted(by:
            ...
            if commonLoggedHabits.count > 0 {
                ...
                let commonFavoriteLoggedHabits =
                    favoriteLoggedHabits.intersection(commonLoggedHabits)
                ...
                if currentUserRanking < followedUserRanking {
                    message = """
                                    Currently #\(ordinalString(from:
                                                                followedUserRanking)), behind you (#\(ordinalString(from:
                                                                                                                        currentUserRanking))) in \(habitName).\nSend them a
                                    friendly reminder!
                                    """
                } else if currentUserRanking > followedUserRanking {
                    message = """
                                    Currently #\(ordinalString(from:
                                                                followedUserRanking)), ahead of you (#\(ordinalString(from:
                                                                                                                        currentUserRanking))) in \(habitName).\nYou might catch up
                                    with a little extra effort!
                                    """
                } else {
                    message = """
                                    You're tied at \(ordinalString(from:
                                                                    followedUserRanking)) in \(habitName)! Now's your chance
                                   to pull ahead.
                                """
                }
                ...
                message = """
                                Currently #\(ordinalString(from:
                                                            followedUserRanking)), in \(habitName).\nMaybe you should
                                give this habit a look.
                                """
                ...
                message = """
                    This user doesn't seem to have done much yet.
                    Check in to see if they need any help getting started.
                    """
            }
